...
func TestGET_Given200ResponseToResponder_Expect200AsResponse(t *testing.T) {
	...
	assert.Nil(err)
	assert.Equal(body, string(bodyBytes))
...
func TestGET_GivenNilResponseToResponder_ExpectNilAsResponseAndErrorLogOnConsole(t *testing.T) {
	...
	assert.Nil(resp)
	assert.Contains(output, fmt.Sprintf("Dispatcher::dispatch An Error occur send http-request to:  Put %s: request error", prUrl))
}
